static char *Ptxdata = 
    ...
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Compiling /tmp/tmpxft_00006551_00000000-9_reduce.cpp3.i (/tmp/ccBI#.lDAOPm)\n"
    "	//-----------------------------------------------------------\n"
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Options:\n"
    ...
    "\n"
    "	.file	1	\"<command-line>\"\n"
    "	.file	2	\"/tmp/tmpxft_00006551_00000000-8_reduce.cudafe2.gpu\"\n"
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/reduce.cu.ptx", Ptxdata), "tex", offset, &tex, devPtr, &desc, size);
}
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/reduce.cu.ptx", Ptxdata), "tex", offset, tex, devPtr, tex.channelDesc, size);
}
...
#pragma begin reduce.cuh

/*
 * stub for remote call to reduce.
 */
void
dscudareduce(dim3 _gdim, dim3 _bdim, size_t _smemsize, cudaStream_t _stream , int n, int *g_idata, int *g_odata)
{
    int _narg = 3;
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) {
        ...
        // a pointer to a device-address 'dscudaAdrOfUva(g_idata)'.
        _ibvargp = _ibvarg + _rcargc;
        _rcargc++;
        _devptr = (void*)(size_t)dscudaAdrOfUva(g_idata);
        ...
        // a pointer to a device-address 'dscudaAdrOfUva(g_odata)'.
        _ibvargp = _ibvarg + _rcargc;
        _rcargc++;
        _devptr = (void*)(size_t)dscudaAdrOfUva(g_odata);
        ALIGN_UP(_off, __alignof(_devptr));
        _ibvargp->type = dscudaArgTypeP;
        _ibvargp->offset = _off;
        _ibvargp->val.pointerval = (RCadr)_devptr;
...
#if !RPC_ONLY
        ibvDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduce.cu.ptx", Ptxdata), 0, mangledname_,
        ...
        rpcDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduce.cu.ptx", Ptxdata), 0, mangledname_,
...
main(int argc, char **argv)
{
    ...
    dscudareduce(grids, threads, smemsize, NULL, nelem, d_idata, d_odata);
