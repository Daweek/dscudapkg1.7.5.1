...
#ifdef ONEK
__global__
void remof_velforce_kernel(int n3, float *fc, float *a_mass, float *vl,
                     VG_XVEC *atype, int *atype_mat, float hsq,float *ekin1){

	__shared__ float cache [ThreadsPB];
    int indx = threadIdx.x;
	int tid  = threadIdx.x + blockIdx.x * blockDim.x;
	int ti3  = indx + blockIdx.x * blockDim.x;
...
__threadfence();
	...
	if(ti3 < n3/3){
        tmp  = (vl[ti3*3]*vl[ti3*3 ]    +
                 vl[ti3*3+1]*vl[ti3*3+1]+
                 vl[ti3*3+2]*vl[ti3*3+2]) * a_mass[atype_mat[atype[ti3].atype]];
...
#ifdef ONEK
		remof_velforce_kernel<<<BLOCKS,THREADS>>>(n3,d_force,d_amass,d_vl,d_x,d_atypemat,hsqf,d_ekin1);
#else
