static char *Ptxdata = 
    ...
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Compiling /tmp/tmpxft_000063b5_00000000-9_directMultiGPU.cpp3.i (/tmp/ccBI#.js8a2E)\n"
    "	//-----------------------------------------------------------\n"
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Options:\n"
    ...
    "\n"
    "	.file	1	\"<command-line>\"\n"
    "	.file	2	\"/tmp/tmpxft_000063b5_00000000-8_directMultiGPU.cudafe2.gpu\"\n"
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/directMultiGPU.cu.ptx", Ptxdata), "tex", offset, &tex, devPtr, &desc, size);
}
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/directMultiGPU.cu.ptx", Ptxdata), "tex", offset, tex, devPtr, tex.channelDesc, size);
}
 ...
 */
void
dscudagravity_kernel(dim3 _gdim, dim3 _bdim, size_t _smemsize, cudaStream_t _stream , float *m, float (*x)[3], float eps, float (*a)[3], float *p, int nj, int ioff)
{
    int _narg = 7;
    int _ibvgdim[3], _ibvbdim[3];
    IbvArg _ibvarg[7], *_ibvargp;
    RCargs _rcargs;
    RCarg _rcarg[7], *_rcargp;
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) {
...
#if !RPC_ONLY
        ibvDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/directMultiGPU.cu.ptx", Ptxdata), 0, mangledname_,
        ...
        _rcargp->offset = _off;
        _rcargp->val.RCargVal_u.valuei = nj;
        ...
        _rcargp->val.type = dscudaArgTypeI;
        _rcargp->offset = _off;
        _rcargp->val.RCargVal_u.valuei = ioff;
        _rcargp->size = sizeof(int);
        _off += _rcargp->size;
        _gdimrc.x = _gdim.x; _gdimrc.y = _gdim.y; _gdimrc.z = _gdim.z;
        _bdimrc.x = _bdim.x; _bdimrc.y = _bdim.y; _bdimrc.z = _bdim.z;
        rpcDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/directMultiGPU.cu.ptx", Ptxdata), 0, mangledname_,
                                 _gdimrc, _bdimrc, _smemsize, (RCstream)_stream,
                                 _rcargs);
    }
}
  void gravity_kernel(float *m, float (*x)[3], float eps, float (*a)[3], float *p, int n, int ioff);
...
#pragma omp parallel for private(ioff)
    for (idev = 0; idev < ndev; idev++) {
        ...
        ioff = ni * idev;
        cudaSetDevice(idev);
        dscudagravity_kernel(grids, threads, 0, NULL, dev->m, dev->x, (float)eps, dev->a, dev->p, n, ioff);
    }
