static char *Ptxdata = 
    ...
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Compiling /tmp/tmpxft_0000614e_00000000-9_broadcast.cpp3.i (/tmp/ccBI#.6bEgnH)\n"
    "	//-----------------------------------------------------------\n"
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Options:\n"
    ...
    "\n"
    "	.file	1	\"<command-line>\"\n"
    "	.file	2	\"/tmp/tmpxft_0000614e_00000000-8_broadcast.cudafe2.gpu\"\n"
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/broadcast.cu.ptx", Ptxdata), "tex", offset, &tex, devPtr, &desc, size);
}
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/broadcast.cu.ptx", Ptxdata), "tex", offset, tex, devPtr, tex.channelDesc, size);
}
...
bcastperf(int argc, char **argv)
{
    ...
    for (sized = size0; sized < maxsize; sized *= ratio) {
        ...
        for (ndev = ndev0; ndev <= ndevmax; ndev++) { 
            get_cputime(&lt, &st);
