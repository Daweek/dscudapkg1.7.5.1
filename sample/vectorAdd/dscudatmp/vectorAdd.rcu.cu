static char *Ptxdata = 
    ...
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Compiling /tmp/tmpxft_000017d6_00000000-9_vectorAdd.cpp3.i (/tmp/ccBI#.bvJyAm)\n"
    "	//-----------------------------------------------------------\n"
    "\n"
    "	//-----------------------------------------------------------\n"
    "	// Options:\n"
    ...
    "\n"
    "	.file	1	\"<command-line>\"\n"
    "	.file	2	\"/tmp/tmpxft_000017d6_00000000-8_vectorAdd.cudafe2.gpu\"\n"
    ...
    "\n"
    "\n"
    "	.entry _Z6VecAddPfS_S_i (\n"
    "		.param .u32 __cudaparm__Z6VecAddPfS_S_i_A,\n"
    "		.param .u32 __cudaparm__Z6VecAddPfS_S_i_B,\n"
    "		.param .u32 __cudaparm__Z6VecAddPfS_S_i_C,\n"
    "		.param .s32 __cudaparm__Z6VecAddPfS_S_i_N)\n"
    "	{\n"
    "	.reg .u16 %rh<4>;\n"
    "	.reg .u32 %r<13>;\n"
    "	.reg .f32 %f<5>;\n"
    "	.reg .pred %p<3>;\n"
    "	.loc	14	78	0\n"
    "$LDWbegin__Z6VecAddPfS_S_i:\n"
    "	mov.u16 	%rh1, %ctaid.x;\n"
    "	mov.u16 	%rh2, %ntid.x;\n"
    "	mul.wide.u16 	%r1, %rh1, %rh2;\n"
    "	cvt.u32.u16 	%r2, %tid.x;\n"
    "	add.u32 	%r3, %r2, %r1;\n"
    "	ld.param.s32 	%r4, [__cudaparm__Z6VecAddPfS_S_i_N];\n"
    "	setp.le.s32 	%p1, %r4, %r3;\n"
    "	@%p1 bra 	$Lt_0_1026;\n"
    "	.loc	14	82	0\n"
    "	mul.lo.u32 	%r5, %r3, 4;\n"
    "	ld.param.u32 	%r6, [__cudaparm__Z6VecAddPfS_S_i_A];\n"
    "	add.u32 	%r7, %r6, %r5;\n"
    "	ld.global.f32 	%f1, [%r7+0];\n"
    "	ld.param.u32 	%r8, [__cudaparm__Z6VecAddPfS_S_i_B];\n"
    "	add.u32 	%r9, %r8, %r5;\n"
    "	ld.global.f32 	%f2, [%r9+0];\n"
    "	add.f32 	%f3, %f1, %f2;\n"
    "	ld.param.u32 	%r10, [__cudaparm__Z6VecAddPfS_S_i_C];\n"
    "	add.u32 	%r11, %r10, %r5;\n"
    "	st.global.f32 	[%r11+0], %f3;\n"
    "$Lt_0_1026:\n"
    "	.loc	14	83	0\n"
    "	exit;\n"
    "$LDWend__Z6VecAddPfS_S_i:\n"
    "	} // _Z6VecAddPfS_S_i\n"
    "\n";
...
typedef u_long RCadr;

typedef u_long RCstream;

typedef u_long RCevent;

typedef u_long RCipaddr;
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/vectorAdd.cu.ptx", Ptxdata), "tex", offset, &tex, devPtr, &desc, size);
}
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/vectorAdd.cu.ptx", Ptxdata), "tex", offset, tex, devPtr, tex.channelDesc, size);
}
 ...
 */
void
dscudaVecAdd(dim3 _gdim, dim3 _bdim, size_t _smemsize, cudaStream_t _stream , float* A, float* B, float* C, int N)
{
    int _narg = 4;
    int _ibvgdim[3], _ibvbdim[3];
    IbvArg _ibvarg[4], *_ibvargp;
    RCargs _rcargs;
    RCarg _rcarg[4], *_rcargp;
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) {
...
#if !RPC_ONLY
        ibvDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/vectorAdd.cu.ptx", Ptxdata), 0, mangledname_,
        ...
        rpcDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/vectorAdd.cu.ptx", Ptxdata), 0, mangledname_,
...
int main(int argc, char** argv)
{
    ...
    dscudaVecAdd(blocksPerGrid, threadsPerBlock, 0, NULL, d_A, d_B, d_C, N);
